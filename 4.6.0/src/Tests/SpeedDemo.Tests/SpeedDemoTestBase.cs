...
namespace SpeedDemo.Tests
{
    public abstract class SpeedDemoTestBase : AbpIntegratedTestBase<SpeedDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SpeedDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<SpeedDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SpeedDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SpeedDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SpeedDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SpeedDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeedDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SpeedDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeedDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SpeedDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SpeedDemoDbContext, Task<T>> func)
        {
            T result;
